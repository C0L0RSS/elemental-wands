...
public final class ProgressionServerSmoke implements ModInitializer {
    ...
    private void run(MinecraftServer server)throws Exception{
        if(++tick==30){
            ...
            check(WandLoadouts.get(player).get(4).equals("flamethrower"),"Second Basic did not open flexible");
            ...
            check(WandLoadouts.get(player).get(1).equals("flashover")&&SpellBooks.credits(player).get(1)==0&&WandProgression.flux(player)==0,"Free learning/slot/price failed");
            ...
            check(WandLoadouts.get(player).get(3).equals("fire_hop"),"Second Technique did not open slot");
            var before=WandLoadouts.get(player);WandLoadouts.equip(player,"FIRE",3,"flamethrower");check(before.equals(WandLoadouts.get(player)),"Wrong-category equip accepted");
            WandLoadouts.equip(player,"FIRE",1,"fire_hop");check(WandLoadouts.get(player).get(3).equals("flashover"),"Compatible slot swap lost spell");
            SpellBooks.use(player,2,new ItemStack(ModItems.ULTIMATE_SPELL_BOOK));
            WandProgression.purchase(player,"FIRE","meteor");check(WandLoadouts.get(player).get(2).equals("meteor"),"Ultimate book failed");
            ...
            near(xp(WizardAffinity.FIRE),200,"XP serialization");check(SpellBooks.credits(player).get(0)==1&&WandLoadouts.get(player).get(2).equals("meteor"),"Book/loadout serialization");
