...
if __name__=='__main__':
    ...
    frames=[('beam',0.0),('beam',1.2),('beam',1.72),('walk',.3),('slam',1.3),('throw',1.9)]
    ...
    # Torso expansion must not pull the mouth away from the server's fixed head pivot.
    anchor=np.array(bones['head']['pivot'])
    head_error=0
    for t in np.linspace(0,3.6,433):
        m=matrices(clips['animation.fractured_guardian.beam'],float(t))['head']
        head_error=max(head_error,float(np.linalg.norm(m[:3,:3]@anchor+m[:3,3]-anchor)))
    assert head_error < .02, head_error
    report['beam_head_anchor_error']=head_error
