...
public final class GuardianGuardSmokeMod implements ModInitializer {
    ...
    private void run(MinecraftServer server) throws Exception {
        ...
        if(opened>=0 && tick==opened+30) {
            ...
            com.anton.elementalwands.util.MeteorManager.spawnMeteor(world,players.getFirst(),boss.getEntityPos(),35,15);
        }
        ...
        if(opened>=0 && tick==opened+90) {
            float loss=beforeMeteor-boss.getHealth();require(loss>0 && loss<240,"Production meteor did not hit safely: "+loss);
        ...
        if(opened>=0 && tick==opened+196) {
            ...
            beforeMeteor=boss.getHealth();
            for(var player:players)com.anton.elementalwands.util.MeteorManager.spawnMeteor(world,player,boss.getEntityPos(),35,15);
        }
        if(opened>=0 && tick==opened+265) {
            require(boss.isAlive(),"Five simultaneous meteors killed boss");
            require(beforeMeteor-boss.getHealth()>120,"Teammate meteors were swallowed by shared invulnerability: "+(beforeMeteor-boss.getHealth()));
