"""Render the UV-mapped cuboids directly, with orthographic depth buffering.
...
atlas = np.asarray(Image.open(ROOT/'fractured_guardian.png').convert('RGB'))
...
def triangles():
    for cube in mesh:
        ...
        for fi,face in enumerate(cube['faces']):
            q=v[face]
            n=np.cross(q[1]-q[0],q[2]-q[0]); n/=np.linalg.norm(n)
            for indices in [[0,1,2],[0,2,3]]:
                yield q[indices],n,np.array(cube['uvs'][fi])[indices],cube['magic'][fi]
...
def render(yaw,elevation,size=1000):
    ...
    for tri,n,uv,magic in triangles():
        ...
        shade=.70+.45*max(0,np.dot(cam@n,light))+.04*max(0,n[1])
        texcoords=w1[...,None]*uv[0]+w2[...,None]*uv[1]+w3[...,None]*uv[2]
        tx=np.clip((texcoords[...,0]*atlas_w).astype(int),0,atlas_w-1)
        ty=np.clip((texcoords[...,1]*atlas_h).astype(int),0,atlas_h-1)
        sampled=atlas[ty,tx].astype(float)
        color=sampled*shade
        if magic:
            glowing=(sampled[...,1]>sampled[...,0]+8)&(sampled[...,2]>sampled[...,0]-5)&(sampled[...,1]>155)
            color[glowing]=sampled[glowing]*1.12
        color=np.clip(color,0,255).astype(np.uint8)
        region[mask]=z[mask]
        rgb[ymin:ymax+1,xmin:xmax+1][mask]=color[mask]
...
d.text((73,110),'TEXTURE STUDY 02  /  ACTUAL MODEL',font=font(21),fill=(107,109,105))
...
d.text((75,1490),'Weathered stone • Sheltered moss • Ancient carvings • Pale magical fractures',font=font(23),fill=(102,104,100))
sheet.save(ROOT/'model-review.png')
template=(ROOT/'viewer.template.html').read_text()
import base64
(ROOT/'preview.html').write_text(template.replace('__MESH_DATA__',json.dumps(mesh,separators=(',',':'))).replace('__CUBE_COUNT__',str(len(mesh))).replace('__ATLAS_DATA__',base64.b64encode((ROOT/'fractured_guardian.png').read_bytes()).decode()))
